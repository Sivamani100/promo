...
export function SiteFooter() {
  ...
  const footerLinks = {
    ...
    Legal: [
      { name: "Privacy Policy", href: "/privacy-policy", isAbsolute: true },
      { name: "Account Deletion", href: "/delete-account", isAbsolute: true }
    ]
  };
      ...
      <div className="max-w-[1280px] mx-auto px-6 relative z-10 text-white">
        ...
        >
           <div className="space-y-5 max-w-lg text-center lg:text-left">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/5 border border-white/5 rounded-full">
                 ...
                 <span className="text-sm font-semibold text-white tracking-tight leading-none">For Creators & Brands</span>
              </div>
              <h3 className="text-3xl font-bold text-white tracking-tighter uppercase leading-[1.1]">Join the Promo <br /> Creator Network today.</h3>
           </div>
              ...
              <form 
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    alert("Registration successful! Welcome to the Promo network.");
                ...
                <ul className="space-y-[8px]">
                  {links.map(link => (
                    <li key={link.name}>
                      <button 
                        onClick={() => {
                          if ('isAbsolute' in link && link.isAbsolute) {
        ...
        >
          <div className="space-y-2 text-center md:text-left order-2 md:order-1">
            <p className="text-base font-medium text-zinc-500 tracking-tight leading-none">© 2026 Promo Labs. Empowering Shop Owners in AP.</p>
          </div>
        </motion.div>
      </div>
    </motion.footer>
